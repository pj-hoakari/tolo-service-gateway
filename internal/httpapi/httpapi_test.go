...
func TestHealthz(t *testing.T) {
	...
	handler := httpapi.NewHandler(httpapi.NewReadiness())
...
func TestUnknownPathIsNotFound(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	handler := httpapi.NewHandler(httpapi.NewReadiness())
	handler.ServeHTTP(rec, httptest.NewRequest(http.MethodPost, "/greet.v1.GreetService/Greet", nil))
...
func TestReadyzWithoutChecks(t *testing.T) {
	...
	status, body := requestReadyz(t, httpapi.NewHandler(httpapi.NewReadiness()))
...
func TestReadyzWithPassingChecks(t *testing.T) {
	...
	status, body := requestReadyz(t, httpapi.NewHandler(readiness))
...
func TestReadinessCheckReceivesRequestContext(t *testing.T) {
	...
	httpapi.NewHandler(readiness).ServeHTTP(rec, req)
